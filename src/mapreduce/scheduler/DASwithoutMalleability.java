...
public class DASwithoutMalleability extends Scheduler{

	final static String NAME = "DASwithoutMalleability";
	final static Double NUMOFMAPSLOTS = 2.0D;
	final static Double NUMOFREDUCESLOTS = 2.0D;

	public DASwithoutMalleability(VMManager theVMManager,
		...
		for (int i = 0; i < tasklist.size(); i++) {
			...
			if (!mMapSlotOccupanceMap.containsKey(slotid))
				...
				this.mMapSlotOccupanceMap.put(
						slotid,
						Math.max(CloudSim.clock(),
								mMapSlotOccupanceMap.get(slotid))
								+ c.getCloudletLength());
		...
		for (int i = 0; i < tasklist.size(); i++) {
			...
			if (!mReduceSlotOccupanceMap.containsKey(slotid))
				...
				this.mReduceSlotOccupanceMap.put(
						slotid,
						Math.max(CloudSim.clock(),
								mReduceSlotOccupanceMap.get(slotid))
								+ c.getCloudletLength());
			mVMManager.mChangeScheduledTaskCount(slotid, 1);
			tasklist.set(i, c);
		}
